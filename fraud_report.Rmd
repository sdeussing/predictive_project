...
fraud %>% filter(amt < 250) %>%
  ggplot(aes(x = TimeOfDay, y = amt)) + geom_point() + geom_smooth() + 
...
We see an accuracy of 0.856 with our Lasso model. Let's use this model to predict on our test set.
...
We will look at predictors that have a high value for all three categories.
...
We see that amount has the highest gain, cover, and frequency. This supports the conclusions from our Lasso model - amount is the most significant predictor of whether a transaction is fradulent. The hour of the day has the second highest gain, cover, and frequency. Like our exploratory plot showed, hours later in the day are associated with more (and thus a higher likelihood) of a fradulent transaction. We also see some categories within our top 7 predictors in terms of predictive importance; however, these have very little predictive power overall (significantly less than amount).
...
In reflection of our work, and if provided with more time on the project, we would look further into the 'category' variable. There were 14 different categories; therefore, none were very prevalent predictors. Instead, we would want to find about 3 category groups using k-means clustering, and then use these categories in prediction models. We would also do the same with 'job', as there were 163 total jobs.
