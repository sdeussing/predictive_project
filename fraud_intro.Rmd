...
##Displaying that our dataset is unbalanced
...
undersampled_data <- ubBalance(c(fraud$merchant, fraud$category, fraud$amt, fraud$city, fraud$state, fraud$lat,),
                               fraud$is_fraud, 
...
```{r Make train}
sample_0 <- fraud[fraud$is_fraud == 0, ][sample(nrow(fraud[fraud$is_fraud == 0, ]),
                                                       1500, replace = FALSE), ]
sample_1 <- fraud[fraud$is_fraud == 1, ][sample(nrow(fraud[fraud$is_fraud == 1, ]),
                                              1500, replace = FALSE), ]

train_set <- rbind(sample_0, sample_1)
```

```{r Make test}
# need to fix with ids to make sure its truly random
remaining <- fraud[-c(which(fraud$is_fraud == 0)[1:1500],
                      which(fraud$is_fraud == 1)[1:1500]), ]

sample_0_rem <- remaining[remaining$is_fraud == 0, ][sample(nrow(remaining[remaining$is_fraud == 0, ]),
                                                            300, replace = FALSE), ]
sample_1_rem <- remaining[remaining$is_fraud == 1, ][sample(nrow(remaining[remaining$is_fraud == 1, ]),
                                                            300, replace = FALSE), ]

# Combine all samples
test_set <- rbind(sample_0_rem, sample_1_rem)
```
